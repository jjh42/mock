...
defmodule MockTest do
  ...
  test "mock functions with different arity" do
    with_mock String,
      ...
      slice: fn string, _range, _len -> string end do
  ...
  test_with_mock "test_with_mock with context", %{foo: foo}, String, [], reverse: fn _x -> :ok end do
    assert String.reverse(3)
    assert foo == "bar"
    assert called(String.reverse(3))
    refute called(String.reverse(4))
  ...
  test_with_mock "passthrough option", Map, [:passthrough], [] do
